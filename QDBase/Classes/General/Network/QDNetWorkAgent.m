...
- (NSURLSessionDataTask *)dataTaskMethod:(QDNetRequestType)type
    ...
    void (^netWorkSuccessBlock)(NSURLSessionDataTask * _Nonnull, id _Nullable) = ^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        // 3. 缓存当前数据并返回
        if (cachePolicy != QDNetCacheRequestReloadIgnoringCacheData && ![judgeString(responseObject[@"error"]) isEqualToString:@"-1"] && ![judgeString(responseObject[@"error"]) isEqualToString:@"-3"]) {
            
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                [[QDNetWorkAgent sharedNetwork] saveCacheWithResponseObject:responseObject forKey:cacheKey cachePolicy:cachePolicy];
            });
        }
...
- (NSURLSessionDataTask *)requestMethod:(QDNetRequestType)type
    ...
    if (parameters) {
        ...
        if ([parameters isKindOfClass:[NSDictionary class]]) {
            ...
            // 生成cachekey的过滤条件 因为每次登陆token、userId、logId都会变，所以就直接将这些东西清除
            NSArray *filterUserInfoArray = @[@"logId", @"token", @"userId"];
            ...
            for (NSString *key in parametersKeys) {
                ...
                if (![filterUserInfoArray containsObject:key]) {
                    cacheKey = [cacheKey stringByAppendingString:judgeString(parameters[key])];
                }
            }
            
            parameters[@"sign"] = [sign creatNetworkSign];
    ...
    if (cacheTimeout > 0) {
        ...
        if (cachePolicy != QDNetCacheRequestReturnCacheDataNoBindUser) {
    ...
    if (cachePolicy != QDNetCacheRequestReturnCacheDataNoBindUser) {
        cacheDict = (NSDictionary *)[[QDNetWorkAgent sharedNetwork].cache objectForKey:cacheKey];
    } else {
        cacheDict = (NSDictionary *)[[QDCacheManager shareInstance].baseDictCache objectForKey:cacheKey];
    }
    ...
    switch (cachePolicy) {
        ...
        case QDNetCacheRequestReturnCacheDataNoBindUser: {// 与用户不绑定的缓存, 退出登录不清理
            if (object) {
                if (timestamp && ![QDNetWorkAgent ifTimeOut:timestamp cacheTimeOut:cacheTimeOut]) {
                    if(success) success(object, YES);
                    return nil;
...
- (void)handleResponseDataError:(id)responseData {
    ...
    if (errorCode.integerValue == -3 || errorCode.integerValue == -1) {
        // 异地登陆
//        [[NSNotificationCenter defaultCenter] postNotificationName:kUserLoginOtherDeviceNty object:nil];
    }
}
...
- (NSURLSessionDataTask *)postImageWithURLString:(NSString *)URLString
    ...
    if (imageNameCount > imageDataCount) {
        ...
        for (NSInteger i = 0; i < imageDataCount - imageNameCount; i++) {
            [imageNames addObject:[NSString stringWithFormat:@"noNameImage%ld", (long)(i + 1)]];
        }
    }
...
#pragma mark 存入缓存的方法
- (void)saveCacheWithResponseObject:(id)responseObject forKey:(NSString *)cacheKey cachePolicy:(QDNetCacheRequestPolicy)cachePolicy{
    ...
    // 不绑定用户
    if (cachePolicy == QDNetCacheRequestReturnCacheDataNoBindUser) {
        [[QDCacheManager shareInstance].baseDictCache setObject:cacheDict forKey:cacheKey];//YYCache 已经做了responseObject为空处理
    } else {
...
#pragma mark - ============== 缓存 ================
+ (void)clearCache {
    [QDCacheManager clearNetworkCache];
}

- (YYCache *)cache {
    if (!_cache) {
        _cache = [QDCacheManager shareInstance].networkCache;
    }
    return _cache;
}
